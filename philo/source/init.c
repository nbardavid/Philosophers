...
/*   Updated: 2024/03/07 09:43:49 by nbardavi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_rules(t_rules *rules, int argc, char **argv)
{
	...
	{
		write(2, "Error initializing mutexs\n", 26);
		exit(EXIT_FAILURE);
	}
	while (i < rules->nbr)
...
void	set_philo(t_rules *rules)
{
	...
	while (i < rules->nbr)
	{
		...
		{
			write(2, "Error initializing eat_lock mutexs\n", 34);
			exit(EXIT_FAILURE);
		}
		i++;
	}
	if (rules->nbr > 1)
	{
		...
		rules->philo[rules->nbr - 1].left_fork = rules->philo[rules->nbr
			- 1].right_fork;
